...
getData <- function(type, features){
  ...
  xdata <- getTable(paste(type,"/","X_",type,".txt",sep=""),features$V2) 
  
  return (cbind(subject_data,ydata,xdata)) 
}
...
# save dataset into results folder
saveResult(dataset,"dataset")

## 5. Creates a second, independent tidy data set with the average of each variable for each activity and each subject. 
tidydata <- ddply(dataset, .(id, activity), .fun=function(x){ colMeans(x[,-c(1:2)]) })

# Adds "_mean" to colnames
colnames(tidydata)[-c(1:2)] <- paste(colnames(tidydata)[-c(1:2)], "_mean", sep="")

# Save tidy tidydata into results folder
